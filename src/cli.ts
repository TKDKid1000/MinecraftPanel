...
const command = yargs()
    .command(
        ["start [directory]", "run"],
        "Launches a MinecraftPanel server",
        (yargs) => {
            return yargs
    ...
    .command(
        ...
        async (argv) => {
            const dir = path.resolve(
                path.join(process.cwd(), argv.name.replace(" ", "-").toLowerCase())
            )
            ...
            try {
                const answers = await inquirer.prompt([
                    ...
                    },
                    {
                        type: "confirm",
                        message:
                            "By typing 'y' you agree to Mojang's eula - https://account.mojang.com/documents/minecraft_eula",
                        name: "eula"
                    }
                ])
                if (!answers.eula) {
            ...
            minecraftpanel.startServer("java -Xmx${answers.ram}G -Xmx${answers.ram}G -jar server.jar nogui", ${answers.port}, "${answers.hostname}")`
                ...
                const jarWriteStream = fs.createWriteStream(
                    path.join(dir, "minecraft", "server.jar")
                )
